   ...
   private List<INeuron> hiddenLayer;
    ...
    public int getMovement(boolean[] shadowSensorInput) {
        ...
        if(motorType == CtrnnProblem.MOTOR_MOSTEAGER_SEPARATE){
            if(left > right) movement = (-left * 4);
            else movement = (right * 4);
        }else if(motorType == CtrnnProblem.MOTOR_TUGOFWAR){
            movement = ((-left + right) * 4.0);
        }else if(motorType == CtrnnProblem.MOTOR_MOSTEAGER_BIDIRECTIONAL){
            if(Math.abs(left - 0.5) > 
                    Math.abs(right - 0.5)){
                movement = ((left - 0.5) * 8);
            }else movement = ((right - 0.5) * 8);
        }else if(motorType == CtrnnProblem.MOTOR_ADDEDFORCE){
            movement = (((left - 0.5)*4) + ((right - 0.5)*4));
        }
        return (int) Math.round(movement);
    }
